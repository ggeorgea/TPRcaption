...
def batchkronecker(matrix1, matrix2):
	batchsize1 = matrix1.shape[0]
	batchsize2 = matrix2.shape[0]

	#thing = torch.ger(matrix1.view(-1), matrix2.view(-1))
	thing = torch.bmm(matrix1.view(batchsize1,-1,1),matrix2.view(batchsize2,1,-1))
	#thing = thing.reshape(*(matrix1.size() + matrix2.size()))
	#thing = thing.reshape(matrix1.shape[0],*(matrix1.size() + matrix2.size()))
